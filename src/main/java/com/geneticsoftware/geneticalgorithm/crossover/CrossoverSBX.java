...
public class CrossoverSBX extends Crossover{
            ...
            if (random.nextDouble() < crossoverRate) {
                ...
                do {
                    ...
                    for (int j = 0; j < parent1.getChromosomes()[0].getChromosomeSize(); j++) {
                        ...
                        double beta;
                        if (u <= 0.5){
                            beta = Math.pow(2*u, 1/(alpha + 1));
                        }
                        else{
                            beta = Math.pow(0.5*(1 - u), 1/(alpha + 1));
                        }
                        child1[j] = 0.5 * ((1 + beta) * p1 + (1 - beta) * p2);
                        child2[j] = 0.5 * ((1 - beta) * p1 + (1 + beta) * p2);
                    }
                    crossoverAttempts++;
