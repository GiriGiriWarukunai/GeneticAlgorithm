...
public class CrossoverTwoPoint extends Crossover{
            ...
            if (random.nextDouble() < crossoverRate) {
                ...
                do {
                    ...
                    for (int j = 0; j < parent1.getCountChromosomes(); j++) {
                        ...
                        int crossoverPoint1 = random.nextInt(parent1.getChromosomes()[j].getBits().getLength() - 1) + 1; // !!!!!СЛУЧАЙНАЯ ГЕНЕРАЦИЯ ТОЧКИ РАЗРЫВА
                        int crossoverPoint2 = random.nextInt(parent1.getChromosomes()[j].getBits().getLength() - 1) + 1; // !!!!!СЛУЧАЙНАЯ ГЕНЕРАЦИЯ ТОЧКИ РАЗРЫВА

                        while (crossoverPoint1 == crossoverPoint2) {
                            crossoverPoint2 = random.nextInt(parent1.getChromosomes()[j].getBits().getLength() - 1) + 1;
                        }
                        ...
                        tmp = BinaryString.splitAndSwap(parent1.getChromosomes()[j].getBits(), parent2.getChromosomes()[j].getBits(), crossoverPoint1);
