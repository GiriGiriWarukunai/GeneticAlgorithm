...
public class CrossoverKPoint extends Crossover{
    @Override
            ...
            if (random.nextDouble() < crossoverRate) {
                ...
                do {
                    ...
                    for (int j = 0; j < parent1.getCountChromosomes(); j++) {
                        ...
                        ArrayList<Integer> crossoverPoint = new ArrayList<>();
                        for (int l = 0; l < k; l++){
                            int tmpRandom = random.nextInt(parent1.getChromosomes()[j].getBits().getLength() - 1) + 1; // !!!!!СЛУЧАЙНАЯ ГЕНЕРАЦИЯ ТОЧКИ РАЗРЫВА
                            while (crossoverPoint.contains(tmpRandom)) {
                                tmpRandom = random.nextInt(parent1.getChromosomes()[j].getBits().getLength() - 1) + 1;
                            }
                            crossoverPoint.add(tmpRandom);
                        }
                        crossoverPoint.sort(Comparator.naturalOrder());

                        tmp = BinaryString.splitAndSwap(parent1.getChromosomes()[j].getBits(), parent2.getChromosomes()[j].getBits(), crossoverPoint.get(0));
                        for (int l = 1; l < k; l++){
                            tmp = BinaryString.splitAndSwap(tmp.getFirst(), tmp.getSecond(), crossoverPoint.get(l));
                        }
